  ...
  return (
    <div className="page">
      ...
      <div className="page-body">
        ...
        {[['Phone / Email', user?.phone || user?.email], ['Hub', user?.hub_name], ['Role', 'Volunteer']].filter(([,v]) => v).map(([l, v]) => (
        ...
        <button className="btn btn-primary" style={{ width: '100%', maxWidth: 320, height: 44, marginTop: 12 }} onClick={save} disabled={saving}>
          {saving ? <div className="spinner" style={{ width: 16, height: 16 }} /> : 'Save Changes'}
        </button>

        <div className="divider" style={{ marginTop: 24 }} />
        <button className="btn btn-ghost" onClick={logout} style={{ color: 'var(--text-3)', fontSize: 13 }}>Sign out</button>

        <AvatarLightbox src={user?.avatar_url} name={user?.name} open={lightbox} onClose={() => setLightbox(false)} editable onEdit={() => toast('Photo upload coming soon', 'warning')} />
      </div>
    </div>
  );
}
