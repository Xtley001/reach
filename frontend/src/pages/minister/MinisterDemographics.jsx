...
const STATUS_COLORS = {
  ...
  needs_transport: 'var(--amber)',
  not_coming:      'var(--red)',
  message_sent:    'var(--green)',
  unreachable:     'var(--text-2)',
};
...
export default function MinisterDemographics() {
  ...
  useEffect(() => {
    cached('demographics', () => api.getDemographics(), TTL.DEMOGRAPHICS)
              ...
              <PieChart>
                ...
                >
                  {statusData.map((entry, i) => (
                    <Cell key={i} fill={STATUS_COLORS[entry.key] || 'var(--border)'} />
                  ))}
                </Pie>
