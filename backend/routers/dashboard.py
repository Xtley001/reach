...
# P1-2.6: Volunteer dashboard — was missing, causing 404 on VolunteerHome load
...
async def volunteer_dashboard(
    ...
    from ..models import Contact, ContactStatus, ContactStatusCode, FollowUpQueue, FollowUpStatus
    ...
    if not campaign:
        return {"total_contacts": 0, "confirmed": 0, "awaiting": 0, "unreached": 0, "streak_days": 0}
    ...
    for c in contacts:
        if c.statuses:
            code = sorted(c.statuses, key=lambda s: s.updated_at)[-1].status_code
            code_str = str(code).split(".")[-1]
            if code_str == "coming":
                confirmed += 1
            elif code_str in ("message_sent", "undecided"):
                awaiting += 1
            elif code_str in ("not_coming", "unreachable", "wrong_number"):
                unreached += 1

    # Streak: count consecutive days the volunteer added at least one contact
    from collections import defaultdict
    by_day = defaultdict(int)
    for c in contacts:
        by_day[c.created_at.date()] += 1
