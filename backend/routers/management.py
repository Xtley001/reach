    ...
    return {"contacts": [
        {
            ...
            "added_by_id":     str(c.added_by),
            "added_by_name":   vol_map.get(c.added_by, "Unknown"),
        }
        for c in contacts
    ]}
...
@minister_router.get("/volunteers/{volunteer_id}")
async def get_volunteer_detail(
    volunteer_id: str,
    user: User = Depends(require_minister),
    db: Session = Depends(get_db),
):
    vol = db.query(User).filter(
        User.id == volunteer_id,
        User.organisation_id == user.organisation_id,
    ).first()
    if not vol:
    ...
    contacts = db.query(Contact).filter(
        Contact.added_by == vol.id,
        Contact.deleted_at.is_(None),
    ).order_by(Contact.created_at.desc()).all()

    contact_list = []
    for c in contacts:
        latest = db.query(ContactStatus).filter(
            ContactStatus.contact_id == c.id
        ).order_by(ContactStatus.updated_at.desc()).first()
        contact_list.append({
            "id": str(c.id),
            "name": c.name,
            "phone": c.phone,
            "location": c.location,
            "current_status": latest.status_code if latest else "no_answer",
            "needs_transport": c.needs_transport,
            "created_at": c.created_at.isoformat(),
        })
    ...
    return {
        ...
        "phone": vol.phone,
        "email": vol.email,
        "hub_id": str(vol.hub_id) if vol.hub_id else None,
        "status": vol.status,
        "contacts": contact_list,
        "contact_count": len(contact_list),
        "confirmed_count": sum(1 for c in contact_list if c["current_status"] == "coming"),
    }
...
async def demographics(
    ...
    try:
        row_count = db.execute(
            sa_text("SELECT COUNT(*) FROM mv_demographics WHERE organisation_id = :org_id"),
            {"org_id": str(user.organisation_id)}
        ).scalar()
        if row_count == 0:
            db.execute(sa_text("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_demographics"))
            db.commit()
    except Exception:
        pass
    ...
    if hub_id:
        ...
        base_filter.append(Contact.added_by.in_(hub_vol_ids) if hub_vol_ids else Contact.id.is_(None))

    # Location breakdown
    location_counts = db.query(
        Contact.location,
        func.count(Contact.id).label("count")
    ).filter(*base_filter).group_by(Contact.location).order_by(func.count(Contact.id).desc()).limit(20).all()
...
async def list_hubs(
    ...
    # Batch 2: volunteer IDs per hub (active volunteers)
    vol_rows = db.query(User.id, User.hub_id).filter(
        User.hub_id.in_(hub_ids),
        User.role == UserRole.volunteer,
        User.status == UserStatus.active,
    ).all()
    vols_by_hub = {}
    for vid, hid in vol_rows:
        vols_by_hub.setdefault(str(hid), []).append(vid)

    # Batch 3: contact counts per volunteer
    all_vol_ids = [r[0] for r in vol_rows]
    contact_counts = {}
    if all_vol_ids:
        for row in db.query(Contact.added_by, func.count(Contact.id)).filter(
            Contact.added_by.in_(all_vol_ids), Contact.deleted_at.is_(None)
        ).group_by(Contact.added_by).all():
            contact_counts[str(row[0])] = row[1]

    # Batch 4: confirmed contact counts per volunteer
    confirmed_counts = {}
    if all_vol_ids:
        for row in db.query(Contact.added_by, func.count(Contact.id)).join(
            ContactStatus, ContactStatus.contact_id == Contact.id
        ).filter(
            Contact.added_by.in_(all_vol_ids),
            Contact.deleted_at.is_(None),
            ContactStatus.status_code == ContactStatusCode.coming,
        ).group_by(Contact.added_by).all():
            confirmed_counts[str(row[0])] = row[1]

    # Sum per hub
    hub_contact_totals   = {}
    hub_confirmed_totals = {}
    for hid, vids in vols_by_hub.items():
        hub_contact_totals[hid]   = sum(contact_counts.get(str(v), 0) for v in vids)
        hub_confirmed_totals[hid] = sum(confirmed_counts.get(str(v), 0) for v in vids)
    ...
    return {"hubs": [
        {
            ...
            "hub_name":        h.name,
            "hub_zone":        h.zone,
            "hub_location":    h.location if hasattr(h, 'location') else None,
            "leader_name":     leader_by_hub[h.id].name       if h.id in leader_by_hub else None,
            "leader_phone":    leader_by_hub[h.id].phone      if h.id in leader_by_hub else None,
            "leader_avatar":   leader_by_hub[h.id].avatar_url if h.id in leader_by_hub else None,
            "volunteer_count": len(vols_by_hub.get(str(h.id), [])),
            "total_contacts":  hub_contact_totals.get(str(h.id), 0),
            "confirmed_count": hub_confirmed_totals.get(str(h.id), 0),
        }
        for h in hubs
    ]}
